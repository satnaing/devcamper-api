...
exports.createCourse = asyncHandler(
  async (req, res, next) => {
    ...
    if (
      ...
      return next(
        new ErrorResponse(
          `User ID ${req.user.id} is not authorized to add a course to this bootcamp ${bootcamp._id}`,
          401
        )
      );
    }
...
exports.updateCourse = asyncHandler(
  async (req, res, next) => {
    ...
    if (
      ...
      return next(
        new ErrorResponse(
          `User ID ${req.user.id} is not authorized to update course ${course._id}`,
          401
        )
      );
    }
...
exports.deleteCourse = asyncHandler(
  async (req, res, next) => {
    ...
    if (
      ...
      return next(
        new ErrorResponse(
          `User ID ${req.user.id} is not authorized to delete course ${course._id}`,
          401
        )
      );
    }
